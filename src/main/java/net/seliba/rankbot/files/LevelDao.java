...
public class LevelDao {
  ...
  public void giveXP(Member member, Guild guild) {
    ...
    if(currentXp == 0L) {
      guild.getController().addSingleRoleToMember(member, member.getJDA().getRoleById(554734490359037996L)).queue();
    }
  ...
  private void levelUp(Member member, long xpLeft, Guild guild) {
    ...
    if(currentLevel == 35) {
      guild.getController().addSingleRoleToMember(member, member.getJDA().getRoleById(554734662472433677L)).queue();
    } else if(currentLevel == 20) {
      guild.getController().addSingleRoleToMember(member, member.getJDA().getRoleById(554734647893032962L)).queue();
      guild.getController().removeSingleRoleFromMember(member, member.getJDA().getRoleById(554734631866335233L)).queue();
    } else if(currentLevel == 10) {
      guild.getController().addSingleRoleToMember(member, member.getJDA().getRoleById(554734631866335233L)).queue();
      guild.getController().removeSingleRoleFromMember(member, member.getJDA().getRoleById(554734613365391361L)).queue();
    } else if(currentLevel == 5) {
      guild.getController().addSingleRoleToMember(member, member.getJDA().getRoleById(554734613365391361L)).queue();
      guild.getController().removeSingleRoleFromMember(member, member.getJDA().getRoleById(554734490359037996L)).queue();
    }
  ...
  public LinkedList<User> getSortedUsers(JDA jda) {
    ...
    sortedUsers.keySet().forEach(consumerString -> {
  ...
  private static <K> Map<K, Long> sortMapByValue(Map<K, Long> mapToSort) {
    ...
    mapToSort.entrySet().stream().sorted(Entry.comparingByValue()).forEach(entry -> result.put(entry.getKey(), entry.getValue()));
    return result;
  }
